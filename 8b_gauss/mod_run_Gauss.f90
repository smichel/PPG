MODULE run
	...
	SUBROUTINE calculate(matrix) !more parameters as needed
		...
		integer :: lines, rest, request, request2, request3, status(MPI_STATUS_SIZE)
...
! 			print*
		...
		do while (t < iterations)
			...
			if ((maxval(abs(diff(2:numEl-1, 2:lines-1) - chunk(2:numEl-1, 2:lines-1)))) < accuracy) then
				accuracyhit = 1
			endif
